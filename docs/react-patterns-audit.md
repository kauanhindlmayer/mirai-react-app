...
10. **No tests** — **In progress.** Tooling now exists (Vitest + React Testing Library + MSW; see `docs/testing-strategy-plan.md`). Pure-logic coverage landed for `auth-storage.ts`, `lib/utils.ts`, and `api-client.ts`'s interceptor chain (26 tests), hook coverage for `useDraftField`, `useWorkItemContext`, and a representative `queries/` hook (11 more), and component coverage for `ErrorState`, `inline-editable-cell.tsx`, `auth-middleware.ts`, and both auth forms (22 more) — 59 tests total. Writing the auth-form tests surfaced and fixed two real bugs: an unconditional 401 handler that misfired on failed login attempts, and native `required` attributes silently blocking the zod-driven validation messages on empty form submission. The work item dialog/board are still untested — the strategy doc's rollout order tracks what's next.
