...
Escape dismisses the popover without inserting anything. Each popover
option shows the person's avatar; the rendered chip itself is name-only and
always resolves to the mentioned person's *current* display name by user ID
(never a name frozen at creation time). Deleting a chip's text removes it as
a single unit rather than leaving a broken partial mention behind.
...
- [x] A rendered mention chip shows the mentioned user's current display name (name-only, no avatar), resolved by user ID
